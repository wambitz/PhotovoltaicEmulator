...
function wundergroundRequest(latitude, longitude) {
    ...
    $.ajax({
        ...
        success: function (parsed_json) {
            ...
            var stationLimit = Math.min(maxStationsAttempt, parsed_json['location']['nearby_weather_stations']['pws']['station'].length);
            var nearStationsId = [];
            var stationFeatures = [];
            for (var i = 0; i < stationLimit; i++) {
                // create array with near weather stations
                // nearStationsId.push({ "id": parsed_json['location']['nearby_weather_stations']['pws']['station'][i]['id'] });
                nearStationsId.push(parsed_json['location']['nearby_weather_stations']['pws']['station'][i]['id']);
            }

            if (parsed_json['current_observation']['solarradiation'] == "--") {
...
/*///////////////////////////////////////////////////////////////
    ...
    Description: This section is inteded to call plotting
